...
def filter_videos(query, only_favourites, with_archived):
    filtered = [vid for vid in load_asmr_file() if query.lower() in vid]
    if only_favourites:
...
def load_asmr_file() -> List[Video]:
    ...
    for entry in json.load(open(FILENAME, encoding="utf8")):
        videos.append(Video(
            ...
            entry["fav"],
            entry["archived"]))
    return sorted(videos, key=lambda x: x.artist)


def write_asmr_file(videos: List[Video]):
    """Convert videos to a dict and then export to json file."""
    if not videos:
        raise Exception("Sent empty list of videos to write to file")
    json.dump([asdict(v) for v in videos], open(FILENAME, "w", encoding="utf8"), indent=2)
