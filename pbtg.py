...
pos = nx.drawing.spring_layout(g2, scale=2, k=1.5/np.sqrt(len(g2)), iterations=500)
nx.draw_networkx_nodes(g2, pos, node_size=10000, ax=ax)

weights = set(d['weight'] for (_, _, d) in g2.edges(data=True))
for weight in weights:
    edgelist = [(u, v) for (u, v, d) in g2.edges(data=True) if d['weight'] == weight]
    nx.draw_networkx_edges(g2, pos, edgelist=edgelist,
                        width=np.log(weight) * 3, ax=ax)

labelsum = lambda label: sum(d['weight'] for n, d in g2[label].items())
labels = [f'{n}\n{labelsum(n)}' for n in g2.nodes]
edgelabels = dict(zip(g2, labels))
nx.draw_networkx_labels(g2, pos, labels=edgelabels, font_size=20, font_family='sans-serif', ax=ax)
plt.axis('off')
plt.show()
f.savefig(f"pbtg_{tag_to_find}.svg", dpi=10000, facecolor='w', edgecolor='w',orientation='portrait', papertype=None, format=None,transparent=False, bbox_inches=None, pad_inches=0.1) 
